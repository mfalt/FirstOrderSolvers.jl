...
"""
    x,y,k = craig_mr_simple(A,M,N,b,kmin,τ,kmax)
...
function craig_mr_full(A,M,N,f,g,kmin,τ,kmax)
    ...
    x,ŷ,i = craig_mr_simple(A,M,N,b,kmin,τ,kmax)
    y = ŷ - y0
    return x,y,i
end
...
Overwrites `x` and `y`.
"""
function craig_mr_full!(x,y,A,M,N,f,g,kmin,τ,kmax,data,craigdata=CraigData(m,n,kmin))
    # x0 = 0
    y0 = N\g # TODO
    #b = f + A*y0
    A_mul_B!(graigdata.tmp1, A, y0) #use tmp1 to store b, this is safe
    graigdata.tmp1 .+= f

    x,y,i,craigdata = craig_mr_simple(x,y,A,M,N,graigdata.tmp1,kmin,τ,kmax,craigdata)
    y .= y .- y0 # y = ŷ - y0
    return x,y,i,craigdata
end
...
function CraigData(m,n,kmin)
    ...
    v = Array{Float64,1}(n)
    fifo = FIFO(kmin)
    return CraigData(m,n,d,d̄,rhs1,rhs2,u,v,fifo)
end
...
Using at least `kmin` iterations and maxmum `kmax`.
"""
function craig_mr!(x,y,A,M,N,b,kmin,τ,kmax,craigdata=CraigData(m,n,kmin))
